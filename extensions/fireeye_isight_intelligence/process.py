...
class MainApp(ImporterProcess):
    ...
    def download(self):
        ...
        while since_timestamp < until_timestamp:
            ...
            try:
                reports_chunk = get_reports_list(
                    api_url, "report/index", public_key, private_key, verify_ssl, 'fireeye', params
                )
        ...
        report_ids = set()
        # set a default timestamp, but there's 0.0002% it will be ever submitted with a blob
        ...
        for item in reports_list:
            ...
            report = get_report(
                api_url, public_key, private_key, item["reportId"], verify_ssl, 'ext_type'
            )
            if not report:
                continue
        ...
        self.save_raw_data(reports_list)
        self.send_info(
            {
                "code": "INF-0001",
    ...
    def transform(self, raw_data):
        ...
        try:
            data_to_transform = transform_reports(json.loads(raw_data)).decode()
        except ValidationError as ex:
            self.send_error(
                {
                    "code": "ERR-0000",
                    "message": "Validation error while transforming data",
                    "description": f"Fireeye isight intelligence failed to transform data{data_to_transform} , error : {ex.messages}",
                }
            )
